...
if __name__ == "__main__":
    ...
    # set waveguide设置器件参数
    waveguide = Waveguide(
        xlength=90, ylength=10, zlength=100, x=150, y=100, z=50, refractive_index=3.47, name="waveguide",
        grid=grid
    )
    waveguide.rotate_Z(angle=50, center=[80, 100, 100])
    grid.add_object(waveguide)

    grid.save_fig(axis="z", axis_number=25)
    grid.save_fig(axis="z", axis_number=50)
    grid.save_fig(axis="z", axis_number=75)
    grid.plot_n(grid=grid, axis="z", axis_index=0)