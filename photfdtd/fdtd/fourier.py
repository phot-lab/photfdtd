...
class FrequencyRoutines:
    ...
    def compute_padding(self, input_data, dt, freq_window_tuple=None, fft_num_bins_in_window=None,
        ...
        if (freq_window_tuple == None):
            ...
            self.end_freq = (input_length / 2.0) / (dt * input_length)  # off by one?
        else:
            self.begin_freq, self.end_freq = freq_window_tuple

        end_time = input_length * dt

        if (fft_num_bins_in_window == None and not fft_bin_freq_resolution == None):
            fft_num_bins_in_window = ((self.begin_freq - self.end_freq) / fft_bin_freq_resolution)
        ...
        required_padding = ceil(fft_num_bins_in_window / ((self.end_freq - self.begin_freq) * dt)) - input_length
    ...
    def export_touchstone_s2p(self):
        raise NotImplementedError
