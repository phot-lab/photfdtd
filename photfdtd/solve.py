...
class Solve:
    ...
    def __init__(
        ...
        for i in range(len(self.grid.objects)):
            geo[
                ...
                self.grid.objects[i].z.start : self.grid.objects[i].z.stop,
            ] = np.sqrt(self.grid.objects[i].permittivity)
        self.geometry = geo

    def _plot_(self, axis: str = "x", index: int = 0, filepath: str = ""):
        """
        绘制截面图。
        :param: axis: 哪个轴的截面
        :param: index: 轴上哪点
    ...
    def _calculate_mode(
        ...
        # 调用phisol包，计算模式
        P, _ = ps.eigen_build(
            self.k, self.n, self.grid.grid_spacing * 1e6, self.grid.grid_spacing * 1e6
        )
        beta_in = 2.0 * np.pi * neff / lam
        beta, Ex, Ey = ps.solve.solve(P, beta_in, neigs=neigs)

        if self.axis == "x":
            Ex_fields = [
                np.reshape(E_vec, (self.grid.Ny, self.grid.Nz)) for E_vec in Ex
            ]
            Ey_fields = [
                np.reshape(E_vec, (self.grid.Ny, self.grid.Nz)) for E_vec in Ey
            ]
        elif self.axis == "y":
            Ex_fields = [
                np.reshape(E_vec, (self.grid.Nx, self.grid.Nz)) for E_vec in Ex
            ]
            Ey_fields = [
                np.reshape(E_vec, (self.grid.Nx, self.grid.Nz)) for E_vec in Ey
            ]
        elif self.axis == "z":
            Ex_fields = [
                np.reshape(E_vec, (self.grid.Nx, self.grid.Ny)) for E_vec in Ex
            ]
            Ey_fields = [
                np.reshape(E_vec, (self.grid.Nx, self.grid.Ny)) for E_vec in Ey
            ]
        else:
            raise RuntimeError("axis should be x, y or z! ")
    ...
    def _sweep_(self, steps: int = 5, lams: list = []):
        ...
        for i in range(steps):
            ...
            env_index += np.sqrt(
                np.amax(self.grid.inverse_permittivity)
                * np.amax(self.grid.inverse_permeability)
            )
        ...
        Ex = np.reshape(
            Ex, (self.neigs, self.Ex_fields[0].shape[0] * self.Ex_fields[0].shape[1])
        )
        Ey = np.array([np.ravel(E_field) for E_field in self.Ey_fields])
        Ey = np.reshape(
            Ex, (self.neigs, self.Ey_fields[0].shape[0] * self.Ey_fields[0].shape[1])
        )
        ...
        # TODO：如何确定哪个object是通光的？假如区域中既有Si又有SiO2，怎么判断哪个才是通光的材料？从而正确地频率扫描？
        # 这句代码仅作测试，没有物理意义！
        ...
        for i in range(steps):
            k = 2.0 * np.pi / lams[i]
            P, _ = ps.eigen_build(
                k, n[i], self.grid.grid_spacing * 1e6, self.grid.grid_spacing * 1e6
            )

            # TODO: Fix complex casting warning
            # 为什么要计算neigs个模式
            beta, Ex, Ey = ps.solve.solve(
                P, 2.0 * n_trial[i] * np.pi / lams[i], E_trial=E_in, neigs=neigs
            )

            Ey_plot.append(
                [
                    np.reshape(
                        E_vec, (self.Ex_fields[0].shape[0], self.Ex_fields[0].shape[1])
                    )
                    for E_vec in Ey
                ]
            )
        ...
        for i in range(steps - 1):
            ...
            E_trace.append(
                np.einsum("kij, kl", Ey_plot[i], prod_next)
            )  # New reordering method. WIP
